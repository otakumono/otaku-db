...
var OtakuDB = (function () {
    ...
    OtakuDB.prototype.updateContent = function (type, idName, content, callback) {
        if (Validation.isMongoId(idName)) {
            return CommonDB.updateOne(this.model(type), { '_id': idName }, content, callback);
        }

        var that = this;
        return this.searchContent(type, idName, function (error, response) {
            if (error) {
                return callback(error, response);
            }

            return that.updateContent(type, response.result._id, content, callback);
        });
    };
    ...
    OtakuDB.prototype.removeContent = function (type, idName, callback) {
        if (Validation.isMongoId(idName)) {
            return CommonDB.removeOne(this.model(type), { '_id': idName }, callback);
        }
        ...
        return this.searchContent(type, idName, function (error, response) {
            ...
            return that.removeContent(type, response.result._id, callback);
        });
    };
